        ...
        for file_idx, file in enumerate(self.files):
            ...
            with open(self.data_path + file, "r") as f:
                ...
                for line in lines:
                    ...
                    tokens = input.split() + output.split()
                    for token in tokens:
                        # do not add name tokens to vocab
                        if not contains_digit(
                                token) and '@' not in token and 'http' not in token and 'www' not in token:
...
class SequencePairDataset(Dataset):
    def __init__(self,
        ...
        if len(self.files) == 0:
            ...
            with open(self.data_path + self.files[0], "r", encoding='utf-8') as pair_file:
                self.data = pair_file.readlines()
    ...
    def __getitem__(self, idx):
        ...
        input, output = tuple(self.data[idx].split('\t'))
        input_token_list = input.split()
        output_token_list = output.split()
